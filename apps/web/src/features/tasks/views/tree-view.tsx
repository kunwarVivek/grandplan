	...
	return (
		...
		>
			...
			<button
				{...attributes}
				{...listeners}
				className="shrink-0 cursor-grab text-muted-foreground opacity-0 transition-opacity active:cursor-grabbing group-hover:opacity-100"
			>
			...
			{/* Priority */}
			<span
				className={cn("shrink-0 font-medium text-xs", priorityConfig.color)}
			>
				{priorityConfig.icon}
			</span>
					...
					<DragOverlay>
						{activeNode && (
							<div className="flex items-center gap-2 rounded border bg-card p-2 shadow-lg ring-2 ring-primary">
								...
								<span className="font-medium text-sm">{activeNode.title}</span>
							</div>
						)}
					</DragOverlay>
				</DndContext>
